...
create procedure find_order(in product_name varchar(40))
	select Customer.customerNo as customerNo, Customer.customerName as customerName,
		FindOrder2.orderNo as orderNo, FindOrder2.quantity as quantity, FindOrder2.OrderSum as orderSum
	from Customer, (
		select OrderMaster.customerNo as CustomerNo, FindOrder.orderNo as orderNo,
			FindOrder.quantity as quantity, FindOrder.OrderSum as OrderSum
		from OrderMaster, (
			select OrderDetail.orderNo as orderNo, OrderDetail.quantity as quantity,
				OrderDetail.quantity * OrderDetail.price as OrderSum
			from OrderDetail, (
				select Product.productNo as productNo
				from Product
				where Product.productName = product_name) as FindProduct
			where OrderDetail.productNo = FindProduct.productNo) as FindOrder
		where OrderMaster.orderNo = FindOrder.OrderNo) as FindOrder2
	where Customer.customerNo = FindOrder2.CustomerNo
    order by orderSum desc;
    
call find_order("32M DRAM");

drop procedure if exists find_predecessor;
create procedure find_predecessor(in employee_no char(8))
	select employeeNo, employeeName, gender, hireDate, department
    from Employee, (
		select hireDate as findDate, department as findDepartment
        from Employee
        where employeeNo = employee_no) as findEmployee
	where department = findDepartment and hireDate < findDate;

call find_predecessor("E2008005");

set global log_bin_trust_function_creators=TRUE;

drop function if exists get_avg_price;
DELIMITER //
create function get_avg_price(product_name varchar(40))
	returns numeric(7,2)
    begin
	declare totalQuantity numeric(7,2);
    declare totalPrice numeric(7,2);
		select sum(quantity), sum(price * quantity) into totalQuantity, totalPrice
        from OrderDetail
        where productNo in (
			select productNo
            from Product
            where productName = product_name);
	return round(totalPrice / totalQuantity, 2);
    end //
DELIMITER ;

select productName, get_avg_price(productName) as avgPrice from Product;

drop function if exists get_total_quantity;
DELIMITER //
create function get_total_quantity(product_no char(9))
	returns int
    begin
...
select productNo, productName, get_total_quantity(productNo) as totalQuantity
from Product
where get_total_quantity(productNo) > 4;

drop trigger if exists set_price;
DELIMITER //
create trigger set_price before insert on Product for each row
	begin
...
insert Product values('P20090007','什么玩意','图书',800.00);
insert Product values('P20100008','不会起名','设备',1200.00);

drop trigger if exists update_salary;
DELIMITER //
create trigger update_salary before insert on OrderMaster for each row
	begin
	declare newHireDate datetime;
		select hireDate into newHireDate from Employee where Employee.employeeNo = new.employeeNo;
        if newHireDate < '19920101'
			then update Employee set salary = salary * 1.08 where Employee.employeeNo = new.employeeNo;
            else update Employee set salary = salary * 1.05 where Employee.employeeNo = new.employeeNo;
		end if;
	end //
DELIMITER ;

select employeeNo, employeeName, hireDate, salary from Employee
where employeeNo = 'E2005001' or employeeNo = 'E2005003';
insert OrderMaster values('202112240007','C20050001','E2005001','20211224',0.00,'I000000020');
insert OrderMaster values('202112250001','C20050001','E2005003','20211225',0.00,'I000000030');
select employeeNo, employeeName, hireDate, salary from Employee
where employeeNo = 'E2005001' or employeeNo = 'E2005003';