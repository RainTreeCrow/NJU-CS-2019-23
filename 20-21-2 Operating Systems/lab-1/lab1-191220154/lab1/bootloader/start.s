...
start:
	...
	movw %ax, %es
	movw %ax, %ss
    cli # closing interuption
    inb $0x92, %al # setting up A20 bus
    orb $0x02, %al
    outb %al, $0x92
    data32 addr32 lgdt gdtDesc # loading GDTR
    movl %cr0, %eax
    orb $0x01, %al
    movl %eax, %cr0 # setting CR0 to switch to protected mode
    data32 ljmp $0x08, $start32 # long jump to execute start32
