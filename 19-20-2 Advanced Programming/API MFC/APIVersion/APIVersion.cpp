...
INT_PTR CALLBACK    Plus(HWND, UINT, WPARAM, LPARAM);
INT_PTR CALLBACK    C_coord(HWND, UINT, WPARAM, LPARAM);
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_APIVERSION, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_APIVERSION));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_APIVERSION));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_APIVERSION);
    ...
    case WM_DISTANCE:
        ...
        for (int i = 0; i < count; i++) {
            for (int j = 0; j < count; j++) {
                if (i == j)
                    continue;
                double temp = pow(points[i][0] - points[j][0]) + pow(points[i][1] - points[j][1]);
                if (temp > distance)
                    distance = temp;
            }
        }
