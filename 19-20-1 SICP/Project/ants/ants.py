...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if not isinstance(insect, QueenAnt) or insect.is_impostor:
                # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        if self.place.bees:
            ...
            while not find_place.bees and find_place.entrance.name != 'Hive':
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, beehive):
        if self.place.bees:
            ...
            find_place = self.place
            i = 0
            while not find_place.bees and find_place.entrance.name != 'Hive' and i < self.max_range:
                find_place = find_place.entrance
                i += 1
            return random_or_none(find_place.bees)
    # END Problem 4
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.is_impostor:
            ...
            find_place = self.place
            while find_place.exit is not None:
                find_place = find_place.exit
                if find_place.ant:
                    if not find_place.ant.is_doubled:
                        find_place.ant.damage *= 2
                        find_place.ant.is_doubled = True
                    if isinstance(find_place.ant, BodyguardAnt) and find_place.ant.contained_ant \
                            and not find_place.ant.contained_ant.is_doubled:
                        find_place.ant.contained_ant.damage *= 2
                        find_place.ant.contained_ant.is_doubled = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        find_insects = []
        find_place = self.place
        while find_place.entrance.name != 'Hive':
            if find_place.bees:
                for b in find_place.bees:
                    find_insects.append(b)
            if find_place.ant:
                find_insects.append(find_place.ant)
            find_place = find_place.entrance
        return find_insects
        # END Problem OPTIONAL
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return self.damage - distance * self.dis - self.insects_shot * self.battery
        # END Problem OPTIONAL
